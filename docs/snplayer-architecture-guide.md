...
| 能力 | 技术实现 | 说明 |
|------|----------|------|
| 加密 | AES-256-CTR + PBKDF2-HMAC-SHA256（10 万次迭代） | 64B 文件头，兼容 MewTool `.enc` 格式 |
...
> **划重点：** `.enc` 文件格式完全开放——文件头包含明文 IV（16B）+ Salt（48B），任何语言均可按照相同参数解密。这意味着即便 SnPlayer 不再维护，用户的视频仍然可以通过第三方工具恢复。
...
2. **PBKDF2 密钥派生**：`PBKDF2(密码, Salt, 100000 次迭代, 256 位)` → AES-256 密钥
3. **写入文件头（64B）**：`[IV: 16B][Salt: 48B]` — 明文存储，解密时读回
...
2. `PBKDF2-HMAC-SHA256(密码, Salt, 100000 次迭代)` → 256 位密钥
