...
- **🔐 军用级加密** — AES-256-CTR + PBKDF2-HMAC-SHA256（10 万次迭代），64 字节文件头保证跨语言兼容
- **🎬 内置播放器** — 解密到临时缓存后直接播放，退出自动清理，无需外部播放器
...
| 功能 | 说明 |
|------|------|
| **视频加密** | 从系统相册多选视频 → AES-256-CTR 流式加密 → 生成 `.enc` + `.tenc` |
| **加密播放** | 解密到缓存 → 内置播放器 → 退出 30s 自动清理临时文件 |
| **解密导出** | `.enc` → 原始 MP4 导出到 `UnLockVideo/` 目录 |
| **缩略图** | 提取视频帧（280×150 JPEG 80%质量）→ 加密存储 → 分批加载 |
| **文件夹** | 创建/重命名/改色/删除，视频自由移动 |
| **安全删除** | 零覆写 + 指数退避重试（3s→6s→12s→24s→30s） |
| **缓存管理** | 自动清理超时播放缓存 + 孤儿缩略图检测 |
...
offset 32-63:  Reserved (32 bytes, 全零)
